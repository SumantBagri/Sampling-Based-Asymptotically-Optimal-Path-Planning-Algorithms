...
def test(map_idx, hw):
    ...
    for i, s in enumerate(states):
        ...
        data_row = [p[1] for p in sorted(data_dict.items(), key = lambda p: CSVOperator.dcols.index(p[0]))]
...
def evaluation():
    ...
    for hw in tqdm(cfg["fmt"]["heuristic_weights"], position=0, leave=False):
        ...
        for _, map_dict in tqdm(cfg["maps"].items(), position=1, leave=False):
            p = map_dict["path"]
            splits = p.split(".")[-2].split("_")
            map_idx = splits[-2]
            rgbworld = load_image(p)
            planner = FMTPlanner(world=rgbworld,
                                 n_samples=1000, # change for sample-based analysis,
                                 max_iter=cfg["max_num_steps"],
                                 col_dst=cfg["col_dst"],
                                 pr=cfg["fmt"]["path_res"])
            for run_num in tqdm(range(cfg["runs_per_map"]), position=2, leave=False):
                sp = map_dict[f"s{run_num}"]['start']
                tp = map_dict[f"s{run_num}"]['target']
                data_dict = {
