...
####################################################################################################################################
#
# OMPL BENCHMARKING CODE FOR OMPL.APP GUI
#
# Stanford University, Dept. of Aeronautics & Astronautics
# Autonomous Systems Laboratory
#
# Authors:	Edward Schmerling, Joseph A. Starek
...
# List planning algorithm names and ID's here (as listed in OMPL.app GUI, starting at 0)
BFMT 		= "BFMT"
BKPIECE 	= "BKPIECE"
FMT 		= "FMT"
LBKPIECE 	= "LBKPIECE"
...
# Set global defaults
pathroot 	= "../resources/"				# Path to configuration files
dflt_config 	= "3D/alpha-1.5.cfg"				# Default config file
dflt_timelimit 	= 1000.0					# Default time limit [s]
dflt_plannerID 	= FMTID						# Default planner

# Change run settings here!!!
dryrun 		= False						# If True, run OMPL App GUI normally (i.e. via buttons -- don't save data; ignore all custom code)
output_file	= "Results/RRTstar.txt"				# Output filename, including path and extension (if no path, saved in current directory)
#output_file	= "Results/Starek.Schmerling.ea.ICRA15.txt"	# Output filename, including path and extension (if no path, saved in current directory)
run_configs 	= ["2D/BugTrap_planar.cfg", 
		   "2D/Maze_planar.cfg", 
		   "3D/cubicles.cfg", 
		   "3D/alpha-1.5.cfg"]				# List of problem configs to run (must be a subset of "configs"; enter [] for nothing)
run_plannerIDs	= [RRTstarID]					# List of algorithms to run (must be a subset of "plannerIDs")
#run_plannerIDs	= [FMTID, BFMTID, RRTstarID, RRTConnectID, 
#		   PRMstarID, SBLID, BKPIECEID, LBKPIECEID]	# List of algorithms to run (must be a subset of "plannerIDs")
run_RMs 	= [1.0]						# List of radius multipliers to run for radius-multiplier-enabled planners (must be non-empty)
Nruns 		= 10						# Number of runs per config/plannerID/RM case, to average out machine noise

# Set the list of planning problems ("configurations") and algorithms here
configs 	= ["2D/BugTrap_planar.cfg", "2D/Maze_planar.cfg", "3D/cubicles.cfg", "3D/alpha-1.5.cfg"]# List of possible problem configurations
plannerIDs 	= [FMTID, BFMTID, RRTstarID, RRTConnectID, PRMstarID, SBLID, BKPIECEID, LBKPIECEID]	# List of possible algorithm ID numbers
algorithms 	= [FMT, BFMT, RRTstar, RRTConnect, PRMstar, SBL, BKPIECE, LBKPIECE]			# List of possible algorithm names (must match those in OMPL.app)
algs_ranged 	= [RRTstar, RRTConnect, SBL, BKPIECE, LBKPIECE]						# List of range-enabled algorithms (must be a subset of "algorithms")
algs_RM 	= [FMT, BFMT, RRTstar, PRMstar]								# List of RM-enabled algorithms (must be a subset of "algorithms")

# Set sample counts for each algorithm for each problem config (enter a "[]" for a planner to skip a particular config)
FMTn1 		= [100, 200, 500, 1000, 1500, 2000, 3000, 4000, 5000, 6000, 8000, 10000, 15000, 20000]	# Bug-trap, Maze, Cubicles
FMTn2 		= [20000, 30000, 50000, 75000, 100000, 150000, 200000]					# Alpha puzzle
FMTn 		= dict(zip(configs, [FMTn1, FMTn1, FMTn1, FMTn2]))
BFMTn		= FMTn
RRTn1 		= [12000, 15000, 20000]		# Bug-trap, Maze
#RRTn1 		= [500, 1000, 2000, 3000, 4000, 5000, 6000, 8000, 10000, 12000, 15000, 20000]		# Bug-trap, Maze
RRTn2		= [2000, 4000, 6000, 8000, 10000, 12000, 15000, 20000, 25000, 30000, 40000]		# Cubicles
RRTn3 		= [20000, 30000, 50000, 75000, 100000, 150000, 200000]					# Alpha puzzle	
RRTstarn 	= dict(zip(configs, [RRTn1, RRTn1, RRTn2, RRTn3]))
RRTConnectn 	= RRTstarn
PRMstarn1 	= [100, 200, 500, 750, 1000, 1500, 2000]						# Bug-trap, Maze, Cubicles
PRMstarn	= dict(zip(configs, [PRMstarn1, PRMstarn1, PRMstarn1, []]))
SBLn 		= PRMstarn
BKPIECEn 	= FMTn
LBKPIECEn	= BKPIECEn

sample_counts 	= [FMTn, BFMTn, RRTstarn, RRTConnectn, PRMstarn, SBLn, BKPIECEn, LBKPIECEn]
Nsamples 	= dict(zip(algorithms, sample_counts))

# Set range values for each algorithm for each problem config
RRTstarr 	= dict(zip(configs, [0.0, 0.0, 0.0, 10.0]))
RRTConnectr 	= RRTstarr
SBLr 		= dict(zip(configs, [0.0, 0.0, 0.0, 0.0]))
BKPIECEr 	= dict(zip(configs, [0.0, 0.0, 0.0, 0.0]))
LBKPIECEr	= BKPIECEr

range_values 	= [RRTstarr, RRTConnectr, SBLr, BKPIECEr, LBKPIECEr]
ranges 		= dict(zip(algs_ranged, range_values))
...
class MainWindow(QtWidgets.QMainWindow):
	...
	if not dryrun:
	    ...
	    if not exists(directory):
    		makedirs(directory)
            self.datafile = open(output_file, 'ab')
	    self.setWindowTitle('OMPL.app [FMT TESTING MODE] (Output File = \"' + output_file + '\")')
	else:
	...
	# run the OMPL App GUI
	    ...
	    for cfg in run_configs:
                window.openConfig(pathroot + cfg)
                for planner_id in run_plannerIDs:
                    self.mainWidget.plannerWidget.geometricPlanning.plannerSelect.setCurrentIndex(planner_id)
                    self.configureApp()
                    self.msgDebug(str(self.omplSetup))

		    name 	= self.omplSetup.getPlanner().getName()
		    Ns 		= Nsamples[name][cfg]
		    if name in algs_ranged:
			self.omplSetup.getPlanner().setRange( ranges[name][cfg] )

#                    if self.omplSetup.getPlanner().getName() == "FMT":
#                        Ns = FMTn
#		    elif self.omplSetup.getPlanner().getName() == "BFMT":
#			Ns = BFMTn
#                    elif self.omplSetup.getPlanner().getName() == "RRTstar":
#                        Ns = RRTn
#                        self.omplSetup.getPlanner().setRange(0.0)
#			if cfg == "../resources/3D/alpha-1.5.cfg":
#                           self.omplSetup.getPlanner().setRange(10.0)
#		    elif self.omplSetup.getPlanner().getName() == "RRTConnect":
#			Ns = RRTConnectn
#			self.omplSetup.getPlanner().setRange(0.0)
#                       if cfg == "../resources/3D/alpha-1.5.cfg":
#                           self.omplSetup.getPlanner().setRange(10.0)
#                   elif self.omplSetup.getPlanner().getName() == "PRMstar":
#                       Ns = PRMn
#	            elif self.omplSetup.getPlanner().getName() == "SBL":
#			Ns = SBLn
			...
			continue
                    ...
                    for n in Ns:
                        self.omplSetup.getPlanner().setNumSamples(n)
                        for rm in run_RMs:
			    if name in algs_RM:
                            	self.omplSetup.getPlanner().setRadiusMultiplier(rm)

                            for i in range(Nruns):
                                self.clear()
				# Display current run information in console
                                print name, 'Run', i, self.configName, ' '.join(self.omplSetup.getPlanner().settings().split('\n')[1:-1]),
				sys.stdout.flush()
				...
				# Store salient run data
                                run_data			= {}
                                run_data["elapsed"]		= elapsed
                                run_data["planner"] 		= name
                                run_data["datetime"] 		= str(datetime.now())
                                run_data["test_config"]		= self.configName
				run_data["collision_checks"] 	= self.omplSetup.getPlanner().getCollisionCheckCount()
				run_data["Nnodes"] 		= self.omplSetup.getPlanner().getNodeCount()
				run_data["Nexplored"] 		= self.omplSetup.getPlanner().getExploredNodeCount()
                                if solved:
                                    run_data["solved"] 		= True
                                    run_data["cost"] 		= self.omplSetup.getSolutionPath().length()
                                else:
                                    run_data["solved"]		= False
                                for param in self.omplSetup.getPlanner().settings().split('\n')[1:-1]:
                                    run_data[param.split(' = ')[0]] = param.split(' = ')[1]

				# Write to JSON file
                                self.datafile.write(json.dumps(run_data) + '\n')
                                self.datafile.flush()

			    if name not in algs_RM:
				break
...
class SolveWidget(QtWidgets.QWidget):
	...
	if not dryrun:
            self.solveButton = QtWidgets.QPushButton('Run Tests!')
	else:
...
if __name__ == '__main__':
    ...
    window = MainWindow()
    window.openConfig(pathroot + dflt_config)
    window.mainWidget.plannerWidget.geometricPlanning.timeLimit.setValue(dflt_timelimit)
    window.mainWidget.plannerWidget.geometricPlanning.plannerSelect.setCurrentIndex(dflt_plannerID)
    window.show()
    sys.exit(app.exec_())
