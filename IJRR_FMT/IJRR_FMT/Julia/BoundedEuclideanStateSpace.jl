...
immutable BoundedEuclideanStateSpace <: StateSpace
    dim::Int64
    lo::Vector{Float64}
    hi::Vector{Float64}
end

function Volume(SS::BoundedEuclideanStateSpace)
    return prod(SS.hi-SS.lo)
end

function FreeVolume(obs::ObstacleSet, SS::BoundedEuclideanStateSpace)
    return Volume(SS) - Volume(obs)
end

function UnitHypercube(d::Int64)
    return BoundedEuclideanStateSpace(d, zeros(d), ones(d))
end

function PairwiseDistances(V::Array{Float64, 2}, SS::BoundedEuclideanStateSpace, r_bound::Float64)
    return pairwise(Euclidean(), V)
end

function PairwiseDistances(V::Vector{Vector{Float64}}, SS::BoundedEuclideanStateSpace, r_bound::Float64)
...
function SampleSpace(SS::BoundedEuclideanStateSpace)
    return SS.lo + rand(SS.dim).*(SS.hi-SS.lo)
end

function PointToState(v::Vector{Float64}, SS::BoundedEuclideanStateSpace)   # for explicit goal sampling
    return v
end

function MotionValidQ(v::Array{Float64}, w::Array{Float64}, obs::ObstacleSet, SS::BoundedEuclideanStateSpace)
...
function StateValidQ(v::AbstractVector, SS::BoundedEuclideanStateSpace)
...
function NearestPoint(w::Vector{Float64}, V::Vector{Vector{Float64}}, SS::BoundedEuclideanStateSpace)
    return findmin(colwise(Euclidean(), hcat(V...), w))
end

function NearestPointsR(w::Vector{Float64}, V::Vector{Vector{Float64}}, r::Float64, SS::BoundedEuclideanStateSpace)
